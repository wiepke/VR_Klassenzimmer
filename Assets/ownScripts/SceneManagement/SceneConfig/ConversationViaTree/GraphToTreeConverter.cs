...
public static class GraphToTreeConverter
{
    ...
    {
        Dictionary<string, StructureTreeNode> result = new Dictionary<string, StructureTreeNode>();
        foreach (Node node in graph.Nodes)
        {
            string nodeId = node.Id;
            List<StructureTreeContainer.NextNode> nextNodes = new List<StructureTreeContainer.NextNode>();
    ...
    private static List<StructureTreeContainer.NextNode> SetNextNodes(Graph graph, Node node)
    {
        ...
        foreach (Edge edgeIn in graph.Edges)
        {
            if (edgeIn.Source == node.Id)
            {
                ...
                {
                    StructureTreeContainer.NextNode nextNode = new StructureTreeContainer.NextNode(edgeIn.Target, ImpulseType.None);
                ...
                else
                {
                    foreach (Edge edgeOut in graph.Edges)
                    {
                        ...
                        {
                            StructureTreeContainer.NextNode nextNode = new StructureTreeContainer.NextNode(edgeOut.Target, SetImpulseType(edgeOut));
                            nextNodes.Add(nextNode);
                        }
                    }
                }
